...
public class MinMaxPlayer:IStrategy
{
    public Play Move(Color color, Table table)
    {
        ...
        {
            possible[i].PlayGame();
            int valor = BestMove(3, int.MinValue,int.MaxValue,color, color == Color.White ? Color.Black : Color.White, copy);
            if (valor > move) (move, moveInd) = (valor, i);
            copy.ResetPlay();
        }
    ...
    private int BestMove(int depth,int alpha,int beta,Color myColor, Color simulationColor, TableCopy table)
    {
        ...
        {
            // Console.WriteLine(item.PositionCurrent+" "+item.PositionMove+" "+depth);
            item.PlayGame();
            move = myColor == simulationColor
                ? Math.Max(BestMove(depth - 1, alpha,beta,myColor, simulationColor == Color.White ? Color.Black : Color.White, table), move)
                : Math.Min(BestMove(depth - 1, alpha,beta,myColor, simulationColor == Color.White ? Color.Black : Color.White, table), move);
            table.ResetPlay();
